...
class Authorization::StubResolverTest < ActiveSupport::TestCase
  ...
  ScopedResource = Struct.new(:department_id, :group_id, :route_id)
  ...
  test "route scope covers a resource on the granted route and denies others" do
    grant = Authorization::Assignment.new(
      role_key: "driver", permission_keys: %w[boarding.manage],
      scope_type: :route, scope_id: "route-1"
    )
    resolver = Authorization::StubResolver.new([ grant ])
    assert resolver.can?("boarding.manage", ScopedResource.new(nil, nil, "route-1"))
    assert_not resolver.can?("boarding.manage", ScopedResource.new(nil, nil, "route-2"))
  end
end
