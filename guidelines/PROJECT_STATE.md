...
| `cafeteria` | Alimentación | **Checkout con bloqueo por alérgeno REAL desde v1.47.0** — `Cafeteria::DietaryRestriction` (propio de este dominio, NO leído de `student_support`; corrección de esta línea, que asumía esa lectura sin que el código la hiciera nunca). Menú/compra/saldo siguen stub (`MenuRoster`/`StudentAccountRoster`, sin modelo `Purchase`), diferido a un futuro incremento de `guidelines/CLOSURE_PLAN.md` Fase D. |
