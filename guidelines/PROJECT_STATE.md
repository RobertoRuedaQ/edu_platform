...
| `student_support` | Bienestar | **Convivencia real desde v1.45.0** (`StudentSupport::DisciplinaryLog`, molde `counseling`, append-only, auditado, `guidelines/CLOSURE_PLAN.md` Fase B) — historia médica (dueño) y acomodaciones **siguen stub**, Clase C, diferidas a propósito. Sensible. Tabla legacy `student_guardians` coexiste con `core.guardian_students` (no migrada; ver §3.2). |
