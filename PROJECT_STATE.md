...
| Campo | Valor |
|---|---|
| **Versión del documento** | `v1.25.0` |
| **Fecha** | 2026-07-16 |
| **Tests** | 528 runs / 0 fallos / 1 skip preexistente (suite completa, en serie — ver `OPEN_PROCESS.md`) |
| **Estado en una línea** | Identidad real + RBAC/entitlement reales + portales de persona + autoservicio de staff + auditoría + CHECKPOINT E + #4 barrido (`teacher_management`/`group_management`/`schedules`-calificaciones/`counseling`) + matrícula por término real (`Schedules::ActiveTermEnrollmentScope`, v1.15.0) + `attendance` (v1.16.0) + `report_cards` (v1.17.0) + `finance` (v1.18.0) + `communication` completo (v1.19.0/v1.20.0) + `assignments` slices 1–3b/4 (v1.21.0–v1.24.0: publicar/ver/calificar + entrega de texto + entregas grupales + adjuntos de entrega) + **`assignments` materiales del docente (ítem #6 del MVP, v1.25.0): mismo molde de tabla puente tenant-scoped que v1.24.0 (`Assignments::Material`, RLS `ENABLE+FORCE`), dueño `Assignment` en vez de `Submission`; la escritura la gatea RBAC (`assignment.manage`, 403 si falta) en vez de una relación de portal; la lectura sigue llegando a estudiante/acudiente por `StudentView`/`GuardianScope` de siempre, y un borrador (o archivada) es inalcanzable gratis porque la tarea misma no está en ese scope; `Assignments::AttachmentTypeCheck` (nuevo) extrae la validación de tipo/tamaño compartida con `AttachmentAdder`**. `LINEAMIENTOS_MVP.md`/`OPEN_PROCESS.md` ordenan lo que sigue (solo queda slice 4: rúbricas). |
...
El changelog completo (`v1.0.0` → `v1.25.0`) vive en **`HISTORIA.md`**. Entrada de esta versión:
